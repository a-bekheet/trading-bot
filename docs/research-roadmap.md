...
- Keep the 29-field contract state under `dimensionless.v10` as the minimum
  model: current per-slot quantity, average entry price, and executable
  unrealized return prevent portfolio-state aliasing. Test them through the
  named `position_state` removal candidate; volatility-regime state still
  belongs once in the market vector.
